...
(function() {
  ...
  function deriveStatusFromRaw(rawState, speedKts) {
    var state = String(rawState || '').toUpperCase();
    if (state.indexOf('AIRBORNE') >= 0 || state.indexOf('IN_FLIGHT') >= 0 || state.indexOf('INFLIGHT') >= 0 || speedKts > 40) return 'AIRBORNE';
    return 'AWAY';
  }
  function normalizeLocationRows(payload) {
    if (Array.isArray(payload)) return payload;
    if (!payload || typeof payload !== 'object') return [];
    var candidates = [payload.rows, payload.data, payload.results, payload.locations, payload.aircraft, payload.items];
    for (var i = 0; i < candidates.length; i += 1) { if (Array.isArray(candidates[i])) return candidates[i]; }
    return [];
  }
  function buildAircraftDetail(rows) {
    var detail = {};
    normalizeLocationRows(rows).forEach(function(row) {
      ...
      var lat = row.latitude; if (lat === undefined) lat = row.lat;
      var lon = row.longitude; if (lon === undefined) lon = row.lon;
      var alt = row.altitude; if (alt === undefined) alt = row.alt_ft;
      var utc = row.last_ping || row.last_seen || row.last_seen_utc || row.utc || '';
      var rawState = row.state || row.status || row.flight_state;
      detail[tail] = {
        lat: lat,
        lon: lon,
        alt_ft: alt,
        speed_kts: speed,
        utc: utc,
        status: deriveStatusFromRaw(rawState, speed || 0)
      };
    });
    return detail;
  }
